...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
    	   <a class="navbar-brand" href="manager.php"><i class="fa fa-dashboard">Dashboard</i></a>
...
<?php
				...
				{		
					...
					echo "$discount";
					$sql3 = "INSERT INTO `customer_service_info`(`date`, `voucher_no`, `service_id`, `service_provider_id`, `discount`) VALUES ('$date','$voucher_no','$service_id','$service_provider_id','$discount')";
				...
				if(isset($_REQUEST['enter']))
				{
					...
					$temp='';
					echo $voucher_no;
					$sql5="SELECT * FROM customer_service_info csi, service s WHERE csi.service_id=s.service_id AND csi.date='$date' AND csi.voucher_no='$voucher_no'";
					...
					$emp_inc='';
					$own_inc='';
					$sql="SELECT * FROM customer_service_info csi, service s, service_provider sp WHERE csi.service_id=s.service_id AND csi.service_provider_id=sp.service_provider_id AND csi.date='$date' AND csi.voucher_no=$voucher_no ";
					...
					while($data = mysqli_fetch_array($res))
					{
						...
						if($discount1==0)
						{
							...
							$em=$price * 0.5;
							$emp=round($em);
							$sql2="SELECT * FROM income_distribution WHERE date='$date' AND service_provider_id='$service_provider'";
							$res2 = mysqli_query($conn,$sql2);
							$numrow = mysqli_num_rows($res2); 
							if($numrow < 1)
							{
								$sql3 = "INSERT INTO `income_distribution`(`date`, `service_provider_id`, `total_income`, `60%_of_total_income`, `40%_of_total_income`) VALUES ('$date','$service_provider','$price','$own','$emp')";
								mysqli_query($conn,$sql3);
								echo "b";
							}
							else if($numrow >= 1)
						...
						else if($discount1!=0)
						{
							...
							if($numrow < 1)
							{
								$sql5 = "INSERT INTO `income_distribution`(`date`, `service_provider_id`, `total_income`, `60%_of_total_income`, `40%_of_total_income`) VALUES ('$date','$service_provider','$temp2','$owner','$empl')";
							...
							else if($numrow >= 1)
							{
								...
								$tot=$total+$temp2;
								$sql6 ="UPDATE `income_distribution` SET `total_income`='$tot',`60%_of_total_income`='$total_own',`40%_of_total_income`='$total_emp' WHERE date='$date' AND service_provider_id='$service_provider'";
								mysqli_query($conn,$sql6);
							}
						}
...
<?php
		...
		$sql2="select * from customer_service_info csi, service s, service_provider sp where csi.service_id=s.service_id and csi.service_provider_id=sp.service_provider_id and csi.date='$date' and csi.voucher_no='$voucher_no' ";
		$res2 = mysqli_query($conn,$sql2);
		  ...
		  <?php
			...
			<input type="hidden" name="service_id" value="<?php echo $row['service_id']; ?>">
             ...
             <td>
              ...
              <td><select  name="select"><option value="0.0">0%</option><option value="0.05">5%</option><option value="0.10">10%</option><option value="0.15">15%</option><option value="0.20">20%</option><option value="0.25">25%</option><option value="0.30">30%</option><option value="0.35">35%</option><option value="0.40">40%</option><option value="0.45">45%</option><option value="0.50">50%</option><option value="0.55">55%</option><option value="0.60">60%</option><option value="0.65">65%</option><option value="0.70">70%</option><option value="0.75">75%</option><option value="0.80">80%</option><option value="0.85">85%</option><option value="0.90">90%</option><option value="0.95">95%</option><option value="1">100%</option></select></td>
              <td>
